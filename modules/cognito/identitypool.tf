...
resource "aws_iam_role" "CognitoRole" {
  name = "CognitoRole"
...
resource "aws_iam_role_policy" "danirolePolicy" {
  name_prefix = "daniPolicy"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
  ...
  role = aws_iam_role.CognitoRole.name
}
...
resource "aws_iam_role_policy_attachment" "s3_full_access_policy_attachment" {
  ...
  role       = aws_iam_role.CognitoRole.id
}
...
resource "aws_cognito_identity_pool_roles_attachment" "example" {
  ...
  roles = {
    "authenticated"   = aws_iam_role.CognitoRole.arn
    "unauthenticated" = aws_iam_role.CognitoRole.arn
  }
}
